import { systemPropList, colorTokenNameList } from '@/constants';
import { ColorPropType, ColorType } from '@/types';
...
export const colorSystemClassName = systemClassName<Record<ColorPropType, Record<ColorType, string>>>({
  variants: colorPropNameList.reduce((variants, propName) => {
    variants[propName] = colorTokenNameList.reduce((props, tokenName) => {
