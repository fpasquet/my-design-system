...
import { spacingTokenNameList, systemPropList } from '@/constants';
import type { MarginPropType, PaddingPropType, SpacingType } from '@/types';
...
>({
  variants: marginPropNameList.reduce(
    (variants, propName) => {
      variants[propName] = spacingTokenNameList.reduce(
        (props, tokenName) => {
...
>({
  variants: paddingPropNameList.reduce(
    (variants, propName) => {
      variants[propName] = spacingTokenNameList.reduce(
        (props, tokenName) => {
          props[tokenName] = `${propName}-${tokenName}`;
